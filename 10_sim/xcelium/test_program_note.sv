...
/* 63  */ assign IMEM[32'h3E] = 32'b000000001000_01010_001_10101_0000011;  // lh x21, 8(x10)   # (Mem[8] = 0x0000ffff) x21 = { 16{Mem[8][15] , Mem[15:0] } = 0xffffffff (sign-extended) /* 67  */
...
/* 66  */ assign IMEM[32'h41] = 32'b000000001000_01010_101_11000_0000011;  // lhu x24, 8(x10)   # (Mem[8] = 0x0000ffff) x24 = { 16'b0, Mem[8][15:0] } = 0x0000ffff (zero-extended)  /* 70  */ 
...
// JAL:                                1101111
                                    ...
                                    0_0000000100_0_00000000_00011_1101111
...
/* 146 */       assign IMEM[32'h92] = 32'h008001EF;  // jal x3, end         # jump to end, x3 = PC (0x248) + 4 = 0x24C (stall for 1 stage)    /* 149-151 */
...
/* 157 */       assign IMEM[32'h9B] = 32'h0421a023;  // sw x2, 0x40(x3)     # mem[0x24C + 0x40 = 0x28C or d652] = 0xABCDE02E     /* 160 */
// /* 158 */       assign IMEM[32'h9C] = 32'hFFDFF06F;  // jal x0, -4          # infinite loop             /* 160 */                      

PC      HEX       ASM
0x0270  100002b7  lui x5, 0x10000      # LED reds base 0x1000_0000
0x0274  10001337  lui x6, 0x10001      # LED greens base 0x1000_1000
0x0278  100023b7  lui x7, 0x10002      # HEX 3-0 base 0x1000_2000
0x027c  10003437  lui x8, 0x10003      # HEX 7-4 base 0x1000_3000
0x0280  100044b7  lui x9, 0x10004      # LCD base 0x1000_4000
0x0284  10010537  lui x10, 0x10010     # Switches base 0x1001_0000
0x0288  100055b7  lui x11, 0x10005     # Keys base 0x1000_5000

0x028c  00f00613  addi x12, x0, 0x00F  # pattern 0x0000_000F
0x0290  00c2a023  sw x12, 0(x5)        # LED reds
0x0294  0f000613  addi x12, x0, 0x0F0  # pattern 0x0000_00F0
0x0298  00c32023  sw x12, 0(x6)        # LED greens

0x029c  00001637  lui x12, 0x1         # tmp = 0x0000_1000
0x02a0  23460613  addi x12, x12, 0x234 # tmp = 0x0000_1234
0x02a4  00c3a023  sw x12, 0(x7)        # HEX 3-0

0x02a8  00005637  lui x12, 0x5         # tmp = 0x0000_5000
0x02ac  67860613  addi x12, x12, 0x678 # tmp = 0x0000_5678
0x02b0  00c42023  sw x12, 0(x8)        # HEX 7-4

0x02b4  04800693  addi x13, x0, 0x48   # 'H'
0x02b8  00d48023  sb x13, 0(x9)
0x02bc  06900693  addi x13, x0, 0x69   # 'i'
0x02c0  00d480a3  sb x13, 1(x9)
0x02c4  02100693  addi x13, x0, 0x21   # '!'
0x02c8  00d48123  sb x13, 2(x9)

0x02cc  0aa00713  addi x14, x0, 0x0AA  # 0xAA
0x02d0  00e29423  sh x14, 8(x5)        # halfword write (aligned)
0x02d4  05500713  addi x14, x0, 0x055  # 0x55
0x02d8  00e300a3  sb x14, 1(x6)        # byte write (lane 1)

loop:
0x02dc  00052783  lw x15, 0(x10)       # read switches
0x02e0  0005a803  lw x16, 0(x11)       # read keys
0x02e4  00f2a023  sw x15, 0(x5)        # mirror switches -> red LEDs
0x02e8  01032023  sw x16, 0(x6)        # mirror keys -> green LEDs
0x02ec  00052883  lw x17, 0(x10)       # read switches
0x02f0  0114A223  sw x17, 4(x9)        # write to LCD
0x02f4  00052903  lw x18, 0(x10)       # read switches
0x02f8  0123A223  sw x18, 4(x7)        # write to HEX 3-0+4 
