...
// ---------------------------------------------------------
// TILELINK MANAGER (Control Logic only)
// ---------------------------------------------------------

    // Tín hiệu Cầu nối (Bridge) từ TileLink Manager sang DMEM
    // Đây là tín hiệu bạn muốn quan sát: Đã qua xử lý TileLink và Address Calculation
    logic   [3:0][13:0]  bridge_dmem_addr;
    logic   [3:0][7:0]   bridge_dmem_wdata;
    logic   [3:0]        bridge_dmem_wen;
    logic   [31:0]       bridge_dmem_rdata;

    tlul_mgr tlul_mgr (
        .clk            (clk),
        .rstn           (rstn),

        // Processor Interface (Kết nối với Proc)
        .req_i          (LSMemM_o),
        .we_i           (MemWriteM_o),
        .addr_i         (ALUResultM_o),
        .wdata_i        ({MemDataWriteM[3], MemDataWriteM[2], MemDataWriteM[1], MemDataWriteM[0]}),
        .be_i           (MemWriteEnM),
        .rdata_o        (MemDataReadM), // Kết quả trả về cho Processor

        // DMEM Interface (Kết nối với DMEM bên dưới)
        .dmem_addr_o    (bridge_dmem_addr),
        .dmem_wdata_o   (bridge_dmem_wdata),
        .dmem_wen_o     (bridge_dmem_wen),
        .dmem_rdata_i   (bridge_dmem_rdata)
    );
//
...
// ---------------------------------------------------------
    dmem dm (
        .clk            (clk),
        .rstn           (rstn),
        .MemWriteEnM    (bridge_dmem_wen),   // Từ TL Manager
        .MemAddrM       (bridge_dmem_addr),  // Từ TL Manager
        .MemDataWriteM  (bridge_dmem_wdata), // Từ TL Manager
        .MemDataReadM   (bridge_dmem_rdata)  // Trả về TL Manager
    );
//
